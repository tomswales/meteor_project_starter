...
Meteor.startup(() => {
    if (Meteor.users.find().count() == 0) {
       Accounts.createUser({username: Meteor.settings.private.login.guest_username, password: Meteor.settings.private.login.guest_password});
    }
    ...
    if (Examples.find().count()== 0) {
        ...
        let two = Examples.insert({created: new Date(), number: 2, text: "Example 2"}, (result) =>{
            Dependents.insert({created: new Date(), text: "Dependent 1", exampleId: two});
            Dependents.insert({created: new Date(), text: "Dependent 2", exampleId: two});
        });
    }
});